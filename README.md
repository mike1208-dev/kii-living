...
| 会社概要         | `/about`      | 代表メッセージ／会社情報テーブル                                  |
| お問い合わせ     | `/contact`    | お問い合わせフォーム／電話・メール・所在地                        |
...
※「無料相談」ボタンは `/contact` に遷移します（`src/data/site.ts` の `consultHref` で変更可）。
...
※ 会社名・代表者名（島﨑 洋子）・設立年月日（2026年7月8日）・所在地・電話番号は設定済みです。
